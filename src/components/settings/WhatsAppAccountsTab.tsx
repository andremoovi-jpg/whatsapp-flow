...
import { Plus, Settings, TestTube2, Trash2, Circle, Wifi, WifiOff, RefreshCw } from 'lucide-react';
      ...
      {accounts?.length === 0 ? (
        ...
        <div className="grid gap-4">
          {accounts?.map((account) => {
            ...
            const usagePercent = Math.min((account.messages_sent_today / account.rate_limit_per_day) * 100, 100);
              ...
              <Card key={account.id}>
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 pt-4 border-t grid grid-cols-3 gap-4 text-sm">
                    <div>
                      <p className="text-muted-foreground mb-1">Rate Limit Diário</p>
                      <div className="flex items-center gap-2">
                        <Progress value={usagePercent} className="flex-1 h-2" />
                        <span className="text-xs font-medium whitespace-nowrap">
                          {account.messages_sent_today.toLocaleString()}/{account.rate_limit_per_day.toLocaleString()}
                        </span>
                      </div>
                    </div>
                    <div>
                      <p className="text-muted-foreground mb-1">Health Check</p>
                      <p className="font-medium flex items-center gap-1.5">
                        <Circle className={cn(
                          'h-2 w-2 fill-current',
                          account.health_status === 'healthy' ? 'text-green-500' : 'text-yellow-500'
                        )} />
                        {account.last_health_check_at
                          ? new Date(account.last_health_check_at).toLocaleString('pt-BR', { 
                              hour: '2-digit', 
                              minute: '2-digit',
                              day: '2-digit',
                              month: '2-digit'
                    ...
                    <div>
                      <p className="text-muted-foreground mb-1">Proxy</p>
                      <p className="font-medium">
                        {account.proxy_enabled ? 'Ativo' : 'Desativado'}
                      </p>
                    </div>
                  </div>
